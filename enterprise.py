...
SCHEMAS.update({'Projects':['id','name','client','location','lead','hse_lead','status','target']+META,'Settings':['id','value']+META,'Standards':['id']+list(REF['standards'][0])+['Review Status']+META,'Evidence':['id','project','record_id','filename','mime','chunk','content']+META,'AlertState':['id','project','status','note']+META})
...
def permitted(role,assigned,project,table='',write=False):
 if role=='Corporate manager': return True
...
def metrics(t,start,end,project=None):
 ...
 controls['Permit']=pct(sum(r.get('Status')=='Active' and dt(r['Expiry Date'])>datetime.now(TZ) and r.get('RAMS Verified')=='Yes' for r in permits),len(permits))
 train=[r for r in scope('10_TRAINING') if r.get('Competency Required')=='Yes']
 controls['Training']=pct(sum(r.get('Result')=='Pass' and (not r.get('Expiry Date') or r['Expiry Date']>=str(today())) for r in train),len(train))
 ...
 al=[a for a in alerts(t) if not project or a['Project']==project]
 cfg=settings(t)
