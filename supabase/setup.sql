...
-- Server-only encrypted account data; operational tables are in database.sql.
begin;
...
comment on table public.hsepulse_organization is
  'Encrypted HSE Pulse memberships, invitation hashes, audit history. Server only; no client policies.';
commit;
